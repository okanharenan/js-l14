...
let lista = document.getElementById("atividades");

for(let i = 0; i < 5; i++){
    let atividade = prompt("Qual atividade deseja adicionar?")
    let prioridade = prompt("Essa atividade é prioritária? (sim/não)")

    if(prioridade.toLocaleLowerCase() === "sim"){
        lista.innerHTML += `<li style = "color: red;">${atividade}</li>`
    } else {
        lista.innerHTML += `<li>${atividade}</li>`
    }
}
